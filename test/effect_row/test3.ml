...
let function_call_test = 
  ...
  let expect_first_node = Node (Root, [Node (EffectName ("Increment", [], [], false), [])]) in
  ...
  let expect_second_node = Node (Root, [Node (EffectName ("Increment", [], [], false), [Leaf])]) in
  ...
  let expect_third_node = Node (Root, [Node (EffectName ("Increment", [], [],false), [Leaf])]) in
