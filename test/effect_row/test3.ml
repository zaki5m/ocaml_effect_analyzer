...
let function_call_test = 
  ...
  let expect_first_node = Node (Root, [Node (EffectName ("Increment", [], []), [])]) in
  ...
  let expect_second_node = Node (Root, [Node (EffectName ("Increment", [], []), [Leaf])]) in
  ...
  let expect_third_node = Node (Root, [Node (EffectName ("Increment", [], []), [Leaf])]) in
