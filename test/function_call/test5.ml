...
let function_call_test = 
  ...
  let expect_first_node =
    Node (Empty, 
      [Node (EffectName "Increment", 
        [Node (FunctionName ("=", [], [LocalVar ("tmp", [], Leaf); ArgsVar ("acc", Leaf)]),
  ...
  let expect_handler = 
    [
      Effc [("_", Node (Empty, [])); 
            ("Decrement", Node (Empty, [Node (FunctionName ("continue", [], [ArgsVar ("k", Leaf); ArgsVar ("eff", Leaf); ArgsVar ("()", Leaf)]), [])]));
            ("Increment", Node (Empty, [Node (FunctionName ("continue", [], [ArgsVar ("k", Leaf); ArgsVar ("eff", Leaf); ArgsVar ("()", Leaf)]), [])]))
          ]; 
      Exnc [("_", Node (FunctionName ("raise", [], [ArgsVar ("e", Leaf); ArgsVar ("()", Leaf)]), []))]; 
      Retc (Node (FunctionName ("unknown", [], [ArgsVar ("_", Leaf); ArgsVar ("()", Leaf)]), []))
    ] 
  in
