...
let function_call_test = 
  ...
  let mul_tree = Node (FunctionName ("mul", [], [LocalVar ("b", [], Leaf);LocalVar ("a", [], Leaf)]), []) in
  let sub_tree = Node (FunctionName ("sub", [], [LocalVar ("b", [], Leaf);LocalVar ("a", [], Leaf)]), [mul_tree]) in
  let add_tree = Node (FunctionName ("add", [], [LocalVar ("b", [], Leaf);LocalVar ("a", [], Leaf)]), [sub_tree]) in
