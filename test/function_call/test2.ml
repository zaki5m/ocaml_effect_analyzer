...
let function_call_test = 
  ...
  let expect_first_node = Node (Empty, [Node (FunctionName ("+", [], [ArgsVar ("b", Leaf);ArgsVar ("a", Leaf)], [ArgVar "a"; ArgVar "b"]), [])]) in
  assert (first = (("add", 2), expect_first_node,[ArgsVar ("b", Leaf); ArgsVar ("a", Leaf)]));
  ...
  assert (second = (("sub", 2), expect_second_node,[ArgsVar ("b", Leaf); ArgsVar ("a", Leaf)]));
  ...
  assert (third = (("mul", 2), expect_third_node,[ArgsVar ("b", Leaf); ArgsVar ("a", Leaf)]));
