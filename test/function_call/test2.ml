...
let function_call_test = 
  ...
  let expect_first_node = Node (Root, [Node (FunctionName ("+", [], [ArgsVar ("b", Leaf);ArgsVar ("a", Leaf)], [ArgVar "a"; ArgVar "b"]), [])]) in
  assert (first = (("add", 2), expect_first_node,[ArgsVar ("a", Leaf); ArgsVar ("b", Leaf)]));
  let second = List.hd (List.tl result) in
  let expect_second_node = Node (Root, [Node (FunctionName ("-", [], [ArgsVar ("b", Leaf);ArgsVar ("a", Leaf)], [ArgVar "a"; ArgVar "b"]), [])]) in
  assert (second = (("sub", 2), expect_second_node,[ArgsVar ("a", Leaf); ArgsVar ("b", Leaf)]));
  let third = List.hd (List.tl (List.tl result)) in
  let expect_third_node = Node (Root, [Node (FunctionName ("*", [], [ArgsVar ("b", Leaf);ArgsVar ("a", Leaf)], [ArgVar "a"; ArgVar "b"]), [])]) in
  assert (third = (("mul", 2), expect_third_node,[ArgsVar ("a", Leaf); ArgsVar ("b", Leaf)]));
  let fourth = List.hd (List.tl (List.tl (List.tl result))) in
  let sub_tree = Node (FunctionName ("sub", [], [LocalVar ("b", [], Leaf);LocalVar ("a", [], Leaf)], [ArgValue; ArgValue]), []) in
  let mul_tree = Node (FunctionName ("mul", [], [LocalVar ("b", [], Leaf);LocalVar ("a", [], Leaf)], [ArgVar "a"; ArgVar "b"]), [sub_tree]) in
  let add_tree = Node (FunctionName ("add", [], [LocalVar ("b", [], Leaf);LocalVar ("a", [], Leaf)], [ArgVar "a"; ArgVar "b"]), [mul_tree]) in
  let expect_fourth_node = Node (Root, [add_tree]) in
