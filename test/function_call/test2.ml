...
let function_call_test = 
  ...
  let expect_first_node = Node (Empty, [Node (FunctionName ("+", [], [ArgsVar ("b", Leaf);ArgsVar ("a", Leaf)]), [])]) in
  assert (first = (("add", 2), expect_first_node));
  let second = List.hd (List.tl result) in
  let expect_second_node = Node (Empty, [Node (FunctionName ("-", [], [ArgsVar ("b", Leaf);ArgsVar ("a", Leaf)]), [])]) in
  assert (second = (("sub", 2), expect_second_node));
  let third = List.hd (List.tl (List.tl result)) in
  let expect_third_node = Node (Empty, [Node (FunctionName ("*", [], [ArgsVar ("b", Leaf);ArgsVar ("a", Leaf)]), [])]) in
  assert (third = (("mul", 2), expect_third_node));
  let fourth = List.hd (List.tl (List.tl (List.tl result))) in
  let sub_tree = Node (FunctionName ("sub", [], [LocalVar ("b", [], Leaf);LocalVar ("a", [], Leaf)]), []) in
  let mul_tree = Node (FunctionName ("mul", [], [LocalVar ("b", [], Leaf);LocalVar ("a", [], Leaf)]), [sub_tree]) in
  let add_tree = Node (FunctionName ("add", [], [LocalVar ("b", [], Leaf);LocalVar ("a", [], Leaf)]), [mul_tree]) in
